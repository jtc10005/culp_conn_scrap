 ...
 */
export function extractNames(fullName: string): { firstName?: string; middleName?: string; lastName?: string } {
  ...
  const cleaned = fullName
    .replace(/\d+/g, '')        // Remove numbers FIRST (like superscript references)
  ...
  if (parts.length >= 2 && parts[0] && parts[1]) {
    ...
    const middleName = firstParts.length > 1 ? firstParts.slice(1).join(' ') : undefined;
  ...
  if (words.length === 0) {
    ...
    if (last && (last.length <= 1 || last === '.') && words.length > 2) {
      ...
      const newMiddle = words.length > 3 ? words.slice(1, -2).join(' ') : undefined;
      return {
        ...(first && { firstName: first }),
...
export function parsePersonFromPage(
    ...
    parentsTable.find('tr').each((_: number, row: Element) => {
      ...
      if (label.includes('father')) {
        ...
        if (parentLinks.length > 0 && parentLinks[0]) {
          ...
          discovered.push({ page: parentLinks[0].page, anchor: `i${parentLinks[0].id}` });
        }
  ...
  personDiv.find('table.grid').first().find('tr').each((_: number, row: Element) => {
    ...
    if (label.startsWith('birth')) {
      ...
      const placeMatch = fullText.match(/\bat\s+([^,]+(?:,\s*[A-Z][a-z\s]+)?)\s*[,.]?/i);
